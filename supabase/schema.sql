...
create index if not exists idx_saved_influencers_user_created on public.saved_influencers (user_id, created_at desc);
...
create policy "users can read own credits" on public.user_credits for
select using (auth.uid () = user_id);

drop policy if exists "users can insert own credits" on public.user_credits;

create policy "users can insert own credits" on public.user_credits for insert
with
    check (auth.uid () = user_id);

drop policy if exists "users can update own credits" on public.user_credits;

create policy "users can update own credits" on public.user_credits for
...
create policy "users can read own campaign emails" on public.campaign_emails for
select using (auth.uid () = user_id);

drop policy if exists "users can insert own campaign emails" on public.campaign_emails;

create policy "users can insert own campaign emails" on public.campaign_emails for insert
with
    check (auth.uid () = user_id);

drop policy if exists "users can update own campaign emails" on public.campaign_emails;

create policy "users can update own campaign emails" on public.campaign_emails for
update using (auth.uid () = user_id)
with
    check (auth.uid () = user_id);
...
drop policy if exists "users can read own smtp config" on public.user_smtp_configs;

create policy "users can read own smtp config" on public.user_smtp_configs for
select using (auth.uid () = user_id);

drop policy if exists "users can insert own smtp config" on public.user_smtp_configs;

create policy "users can insert own smtp config" on public.user_smtp_configs for insert
with
    check (auth.uid () = user_id);

drop policy if exists "users can update own smtp config" on public.user_smtp_configs;

create policy "users can update own smtp config" on public.user_smtp_configs for
update using (auth.uid () = user_id)
with
    check (auth.uid () = user_id);

drop policy if exists "users can read own saved influencers" on public.saved_influencers;

create policy "users can read own saved influencers" on public.saved_influencers for
select using (auth.uid () = user_id);

drop policy if exists "users can insert own saved influencers" on public.saved_influencers;

create policy "users can insert own saved influencers" on public.saved_influencers for insert
with
    check (auth.uid () = user_id);

drop policy if exists "users can delete own saved influencers" on public.saved_influencers;

create policy "users can delete own saved influencers" on public.saved_influencers for delete using (auth.uid () = user_id);